...
Full-stack AI e-commerce store built with Next.js 14, Supabase Auth, Supabase PostgreSQL, Prisma ORM, and Anthropic Claude. Built phase-by-phase from scaffold to production.
...
## Phase 2.5 — Auth Migration: Clerk → Supabase Auth ✅

**Reason:** Clerk does not support phone sign-up from Vietnam. Migrated to Supabase Auth (email/password).

**Files created:**
- `lib/supabase/server.ts` — server-side Supabase client (SSR cookie handling)
- `lib/supabase/client.ts` — browser-side Supabase client
- `middleware.ts` — session refresh on every request
- `app/auth/sign-in/page.tsx` + `SignInForm.tsx`
- `app/auth/sign-up/page.tsx` + `SignUpForm.tsx`

**Files modified:**
- `app/layout.tsx` — removed `ClerkProvider`
- `components/Navbar.tsx` — replaced Clerk hooks with Supabase `onAuthStateChange`
- `app/orders/page.tsx` — replaced Clerk `auth()` with Supabase `getUser()`
- `app/admin/page.tsx` — replaced Clerk admin check with Supabase
- All API routes using auth — now use `createClient().auth.getUser()`
- `.env.local` — removed Clerk keys; `ADMIN_USER_ID` is now the Supabase UUID

**To get your ADMIN_USER_ID:**
1. Sign up at `http://localhost:3000/auth/sign-up`
2. Go to Supabase Dashboard → Authentication → Users
3. Copy your UUID and paste it into `.env.local` as `ADMIN_USER_ID` and `NEXT_PUBLIC_ADMIN_USER_ID`
...
## Phase 4 — Layout Beautification ✅

**Goal:** Transform the app into a polished, professional-looking store.

See `plans/LAYOUT.md` for the detailed plan.
...
3. `vercel deploy` or connect GitHub → Vercel
4. Set all env vars in Vercel dashboard
5. Run `prisma db push` against production DB
6. Verify production URL end-to-end
